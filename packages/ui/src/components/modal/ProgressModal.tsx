...
export default function ProgressModal() {
  ...
  return (
    <div className='fixed top-0 right-0 bottom-0 left-0 z-20 bg-grey-950/40'>
      <div
        className={`absolute top-1/2 left-1/2 flex flex-1 -translate-x-1/2 -translate-y-1/2 transform items-center justify-between gap-10 rounded-lg border border-grey-700 bg-grey-0 px-8 py-6 shadow-strong ${progress.width ?? 'w-[800px]'}`}
        ref={modalRef}
      >
        <div className='flex w-full flex-col gap-4'>
          ...
          <div className='relative flex h-1.5 w-full rounded-full bg-grey-300'>
            <div
              className={`absolute top-0 left-0 z-20 h-1.5 w-full rounded-full bg-primary-500`}
