...
import { toastAtom, toastDefault } from '@repo/ui/src/jotai/modalAtoms';
...
export default function Toast() {
  ...
  return (
    <div className='fixed top-0 right-0 bottom-0 left-0 z-20'>
      ...
      >
        <div
          className={`shadow-light flex max-w-3xs min-w-96 items-center justify-between gap-2 rounded-lg px-4 py-3.5 ${variantStyle[toast.style ?? 'dark'].wrapper}`}
        >
          ...
          <p
            className={`text-14 flex flex-1 leading-20 ${variantStyle[toast.style ?? 'dark'].text}`}
          >
            {toast.text}
          </p>
          <IoClose
