    ...
    >
      {valueList?.map((item) => (
        <button
          ...
          className={`text-14 flex flex-1 items-center justify-center gap-1 rounded-xs px-4 py-2 leading-20 ${variantStyle[isDisabled ? 'disabled' : value.key === item.key ? 'check' : 'uncheck'].button}`}
